...
class TenantResolver
{
            ...
            while ($attempts < self::MAX_RETRIES) {
                try {
                    $response = Http::timeout(5)
                        ...
                        ->get($this->getCentralManagementUrl() . "/api/tenants/{$tenantId}");
    ...
    private function configureTenantDatabase(array $tenantConfig): void
    {
        ...
        Config::set([
            'database.connections.tenant' => [
                'driver' => 'pgsql',
                'host' => $tenantConfig['database_host'] ?? env('TENANCY_DB_HOST', 'postgres-tenants'),
                'port' => $tenantConfig['database_port'] ?? env('TENANCY_DB_PORT', '5432'),
                'database' => $tenantConfig['database_name'],
                'username' => $tenantConfig['database_username'] ?? env('DB_USERNAME', 'laravel_user'),
        ...
        Config::set('cache.prefix', 'tenant_' . $tenantConfig['tenant_id'] . '_');

        // Update session configuration
        Config::set('session.cookie', 'tenant_' . $tenantConfig['tenant_id'] . '_session');
    }
    ...
    {
        return isset($tenantConfig['status']) && $tenantConfig['status'] === 'active';
    }
    ...
    {
        return config('app.service_token', env('SERVICE_TOKEN', 'default-service-token'));
    }
    ...
    {
        return config('app.central_management_url', env('CENTRAL_MANAGEMENT_URL', 'http://central-management'));
    }
